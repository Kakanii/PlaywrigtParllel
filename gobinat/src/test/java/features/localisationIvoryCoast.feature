Feature: Cote D'Ivoire (Ivory Coast) Localisation

  #IvoryCoastLocalisation
  Background: Start the application
    Given Open the login page
    When Wait 10 seconds
    Then Close the hotjar window

  @Smoke @IvoryCoastLocalisation @Localisation @Supplier @QuickSanity 
  Scenario: Verify invite a Supplier whose tax country is IvoryCoast with out questionnaire and verify invited supplier listed in contacted suppliers with pending invites status
    #Open buyer page
    ...
    #select Ivory Coast from the dropdown
    Then Select "Cote D'Ivoire (Ivory Coast)" in "Supplier Country of Registration" dropdown
  ...
  @Smoke @IvoryCoastLocalisation @Localisation @QuickSanity 
  Scenario: Verify maildrop registration link and verify the supplier registration process from maildrop
   ...
   #select Ivory Coast from the dropdown
    ...
    Then Enter "TestAddress1" in "Address Line 1" field after "IvoryCoast" supplier registration
    Then Enter "TestAddress2" in "Address Line 2" field after "IvoryCoast" supplier registration
    Then Enter "TestCity" in "City" field after "IvoryCoast" supplier registration
    #Then Enter "1234" in "PO Box " field after "IvoryCoast" supplier registration
    Then Select "Abidjan" in "District" dropdown
   ...
   @Smoke @IvoryCoastLocalisation @Localisation @Supplier @QuickSanity 
  Scenario: Verify As a Buyer, I should be able to Onboard a Supplier whose Tax Country is IvoryCoast and with out questionnaire
    #Open buyer page
    Then Enter data in Email Address field
    Then Click on Continue button on Login page
    Then Enter data in Password field
   ...
   #invite without questionare
    ...
    Then Verify Tax Country "Cote dIvoire" is Pre populated with country
    ###Labels
    ...
    Then Wait for 3 seconds
    ###tooltips
    Then Show "Company Registration Number" tooltip "Company Registration Number issued by the Center for the Promotion of Investments in Côte d'Ivoire (CEPICI) which is usually minimum 4 to max 20 aphanumeric characters. This is an optional field." under "Tax Details"
    Then Show "Company Registration Document" tooltip "Attach your Company’s Registration Document issued by the Center for the Promotion of Investments in Côte d'Ivoire (CEPICI) in the below mentioned formats.This is a optional attachment." under "Tax Details"
    Then Show "Tax Identification Number" tooltip "Tax/VAT Registration Number issued by the the Corporate - Tax administration in Ivory Coast which is usually minimum 4 to maximum 20 alphanumeric characters. This is a mandatory field." under "Tax Details"
    Then Show "Tax Registration Document" tooltip "Attach your company's Certificate of Tax Registration issued by the Corporate - Tax administration in Ivory Coast in the below mentioned formats.This is a mandatory attachment." under "Tax Details"
    ...
    Then Verify "Stock Corporations" is listed in "Business Type" dropdown under "Tax Details"
    Then Verify "Other" is listed in "Business Type" dropdown under "Tax Details"
    Then Select "Stock Corporations" value from Business type dropdown
    #Date calandar
    Then Show date calander in "Date Established" label
    ####Flag
    Then Verify "Cote dIvoire" flag Phone Number
    And Verify "Cote dIvoire" flag Fax Number
    ...
    Then Verify phone number "Phone Number" is entered in accepted format "+225 (123) 456-7890"
    Then Enter phone number "1234567890" in "Fax Number" field
    Then Verify phone number "Fax Number" is entered in accepted format "+225 (123) 456-7890"
    ...
    #Then Verify "Cote D'Ivoire (Ivory Coast)" listed in "Country/Region" dropdown
    Then Select "Cote D'Ivoire (Ivory Coast)" in "Country/Region" dropdown
    ...
    Then Verify phone number "Primary Phone" is entered in accepted format "+225 (123) 456-7890"
    Then Enter phone number "1234567999" in "Secondary Phone" field
    Then Verify phone number "Secondary Phone" is entered in accepted format "+225 (123) 456-7999"
    ###Required Error
...
@Smoke @IvoryCoastLocalisation @Localisation @Supplier @QuickSanity 
  Scenario: Verify As a Buyer, I should be able to Onboard a Supplier whose Tax Country is Ivory Coast and with questionnaire
    Then Enter data in Email Address field
    ...
    Then Upload "PDF" under "Upload Document"
    Then Click on the Confirm button.
    And Select "Stock Corporations" value from Business type dropdown
    And Enter number "1546457447455" in "Phone Number" in corporation details
    #Then Scroll to the top of the page
  ...
  @Smoke @IvoryCoastLocalisation @Localisation @Supplier @QuickSanity 
  Scenario: Verify As a Buyer, I should be able to Onboard a Supplier whose Tax Country is IvoryCoast and with out questionnaire with Diversity
    #Open buyer page
    Then Enter data in Email Address field
    Then Click on Continue button on Login page
    Then Enter data in Password field
  ...
  @Smoke @IvoryCoastLocalisation @Localisation @Supplier @QuickSanity 
  Scenario: Verify As a Buyer, I should be able to Onboard a Supplier whose Tax Country is IvoryCoast and with out questionnaire with Additional Information
    #Open buyer page
    Then Enter data in Email Address field
    Then Click on Continue button on Login page
    Then Enter data in Password field
  ...
  @Smoke @IvoryCoastLocalisation @Supplier @BankAccount @QuickSanity 
  Scenario: Add Company Account with valid data for Country Ivory Coast for existing supplier
    When Enter Ivory Coast Supplier Email in Email Address field
    ...
    Then Select "Cote D'Ivoire (Ivory Coast)" in "Country/Region" dropdown
    Then Enter value for Bank Name "Bank of Ivory Coast"
    Then Select "West African CFA franc (XOF)" in "Currency" css dropdown
    Then Enter value for Account Holder Name "Ivory Coast"
    #Then Enter value for Bank Routing Number "23634795651"
    And Wait for 2 seconds
    #Then Enter value for Bank Account Number "32535345345474"
    Then Enter input data in "Account Number" field
    And Wait for 2 seconds
    And Enter value for Bank Key "CI32342343"
    Then Enter "Testing" in Bank Desciption
    #When Enter the IBAN evidence for Albania
    #And Enter value for Bank Swift Code "12345678901"
    ...
    Then Enter value for Bank Branch Name "Bank of Ivory Coast"
    ...
    #Then Enter value for Postal Code "4326"
  ...
  @Smoke @IvoryCoastLocalisation @Supplier @BankAccount @QuickSanity 
  Scenario: Add Factoring Company Account with valid data for Country Ivory Coast for existing supplier
    When Enter Ivory Coast Supplier Email in Email Address field
    #When Enter Supplier Email in Email Address field
    Then Click on Continue button on Login page
    Then Enter data in Nokia Password field
    ...
    Then Select "Cote D'Ivoire (Ivory Coast)" in "Country/Region" dropdown under Factoring Company Address
    Then Wait for 3 seconds
    ...
    #Then Enter "43445" in "Postal Code" field under "Factoring Company Address" section
    #Then Select "Blagoevgrad" in "Province" dropdown on Factoring Company Address
