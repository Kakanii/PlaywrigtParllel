Feature: Add New Contact in Management
  ...
  @Regression @addnewcontact @TYSFunctionality 
  ...
  @Smoke @Supplier @addnewcontact @CDCI @SanitySuiteCheck @SanityCheck
  Scenario: As a Supplier, I should add new contact while adding CEO/CFO/DOS
    #Open buyer page
  ...
  @Smoke @Supplier @addnewcontact @CDCI @Sanity
  Scenario: As a Supplier, I should add new contact while adding CEO/CFO/DOS/Add Other
    ...
    Then Switch to the parent window
    ############## Add Other
    Then Click on the "Add Other" button
    Then Select the option "Add a New Contact" on the Assign a Contact to Management Team modal
    Then Enter "Branch" in "First Name" field
    Then Enter "Manager" in "Last Name" field in add contact
    Then Select the "1992" year from the drop down
    Then Enter "MD" in "Job Title" field
    Then Enter "estratreshub@mailinator.com" in "Email ID" field
    Then Enter phone number "9876543212" in "Primary Phone" field
    Then Select "Add a New Location" in "Select Location" dropdown
    Then Select "Branch" in "Address Type" dropdown
    Then Select "United Kingdom" in "Country/Region" dropdown
    Then Enter "Geroginaya" in "City" field
    Then Enter 20 digit alpha numeric data in "Address Line 1" field
    Then Enter 20 digit alpha numeric data in "Address Line 2" field
    Then Enter "B1 2JJ" in "Postal Code" field
    Then Enter "12345678" in "PO Box" field
    Then Enter "B1 2JJ" in "PO Box Postal Code" field
